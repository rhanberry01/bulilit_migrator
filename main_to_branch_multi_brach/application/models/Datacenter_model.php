...
class Datacenter_model extends CI_Model {
	...
	public function distinct_branch($db_batch){
		$this->load_db();
		$this->db->where("throw !=", 1);
		$this->db->where("branch_code IN(".$db_batch.")");
	...
	public function get_batch_db($batch){
		$this->load_db();
		$this->db->where("batch", $batch);
		$result = $this->db->get("batch_branch_updates");	
		$result = $result->result_array();
		return $result;
