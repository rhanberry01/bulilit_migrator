...
class Welcome extends CI_Controller {
	...
	public function index($bool = 1,$batch = null)
	{
			...
			$batch_db = $this->dc->get_batch_db($batch);
			$batch_db_arr = array();
			foreach ($batch_db as $key) {
				$batch_db_arr[] = $key['branch_code'];
			}
			
			$db_batch = "'".implode("','", $batch_db_arr)."'";

		while(true){
			$branches = $this->dc->distinct_branch($db_batch);
