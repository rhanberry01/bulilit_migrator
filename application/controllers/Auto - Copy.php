	...
	public function create_purchase_request(){
        ...
        $min_purchase_piece =  $this->auto->get_frequency(null,$branch_code,$supplier_code);
            if(!empty($min_purchase_piece)) {
                $min_purchase_piece = end($min_purchase_piece);
                if($min_purchase_piece["status"] == 1) $min_purchase_piece = $min_purchase_piece["min_pc"];
            ...
            foreach ($divs as $des) {
                if(isset($item[$des->product_id])){
                    ...
                    $qty = $sugg_po;
                    /*if($code != null){
                        if($item[$des->product_id]['qty'] != 0){
                            $qty =  $po_details[$des->product_id]['qty'];
                        }
                        else{
                            $qty = 0;
                        }

                        if($item[$des->product_id]['sugg_po'] != 0){
                           $sugg_po =  $item[$des->product_id]['sugg_po'];
                        }
                    }*/
                    ...
                    $error_msg = null;
					  ...
					  $po_details = array();
                        ...
                        if($settings["auto_po"] == 1){
                          if($net_total >= $settings["min"] && $net_total < $settings["max"]) $settings["auto_po"] = 1;
